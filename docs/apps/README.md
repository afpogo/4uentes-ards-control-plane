# Seccion Apps

## Proposito

Esta seccion agrupa documentacion cuyo dueno principal es un servicio logico o
un repositorio canonico.

## Usar Esta Seccion Para

- notas por repo/app
- analisis de workflows por app
- notas de contrato de deployment por app
- notas de adopcion de capabilities por app

## Duenos Tipicos

- `4uentes-ards-core` cuando se documenta como fuente estandar consumida
- `sst-bend`
- `4uentes-auth`
- `sst-fend`
- `sst-extension`
- `sst-4uentes-infra` cuando se discute como artefacto de un solo repo

## Catalogos Por Repo

- [4uentes-ards-core.md](4uentes-ards-core.md)
- [4uentes-auth.md](4uentes-auth.md)
- [sst-bend.md](sst-bend.md)
- [sst-chatbot.md](sst-chatbot.md)
- [sst-extension.md](sst-extension.md)
- [sst-fend.md](sst-fend.md)
- [sst-4uentes-infra.md](sst-4uentes-infra.md)

## Referencias Relacionadas Del Control-Plane

- [service-catalog.md](service-catalog.md)
- [documentation-information-architecture.md](../documentation-information-architecture.md)
