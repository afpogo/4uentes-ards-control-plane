...
const COMMENT_FILE = path.join(OUTPUT_DIR, 'jira-sst-30-session-capture-async-followup-comment.md');
...
async function main() {
  ...
  try {
    ...
    const summary = {
      ...
      issueKey: ISSUE_KEY,
      mode: 'approved-write',
      action: 'session-capture-async-followup-comment',
      before,
      after,
...
async function getIssue(client, cloudId) {
  const result = await client.callTool('getJiraIssue', {
    ...
    fields: ['summary', 'status', 'resolution', 'updated', 'labels'],
...
function writeEvidence(summary) {
  const jsonOutput = path.join(OUTPUT_DIR, 'jira-sst-30-session-capture-async-followup-comment-result.json');
  const output = path.join(OUTPUT_DIR, 'jira-sst-30-session-capture-async-followup-comment-summary.md');
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.writeFileSync(jsonOutput, JSON.stringify(sanitizeJson(summary), null, 2), 'utf8');

  const lines = [
    '# SST-30 Jira Session Capture Async Follow-up Comment',
    '',
    '## Estado',
    '',
    `- Fecha: ${today()}`,
