...
const COMMENT_FILE = path.join(OUTPUT_DIR, 'jira-sst-30-closure-prep-comment.md');
...
function writeEvidence(summary) {
  const jsonOutput = path.join(OUTPUT_DIR, 'jira-sst-30-closure-prep-comment-result.json');
  const output = path.join(OUTPUT_DIR, 'jira-sst-30-closure-prep-comment-summary.md');
  fs.mkdirSync(path.dirname(output), { recursive: true });
  fs.writeFileSync(jsonOutput, JSON.stringify(sanitizeJson(summary), null, 2), 'utf8');

  const lines = [
    '# SST-30 Jira Closure Prep Comment',
    '',
    '## Estado',
    '',
    `- Fecha: ${today()}`,
